...
class SortTester {
 ...
 public:
    ...
    void testQuickSort(const std::function<void(std::vector<int>&, int, int)>& quick_sort,
        ...
        for (int n = 500; n <= max_n; n += 100) {
            ...
            for (int it = 0; it < runs; it++) {
                {
                    ...
                    quick_sort(v, 0, static_cast<int>(v.size()));
            ...
            std::cerr << "QuickSort: Done n=" << n << "\n";
        }
    }

    void testIntrosort(const std::function<void(std::vector<int>&, int, int, int)>& introsort,
                       const std::string& prefix) {
        const int max_n = 100000;
        std::ofstream fout_random(prefix + "_random.csv");
        std::ofstream fout_decreasing(prefix + "_decreasing.csv");
        ...
        for (int n = 500; n <= max_n; n += 100) {
            ...
            int depth = 0;
            if (n > 1) depth = 2 * std::floor(std::log2((double)n));

            for (int it = 0; it < runs; it++) {
                {
                    auto v = generator.GetRandomVector(n);
                    auto start = std::chrono::high_resolution_clock::now();
                    introsort(v, 0, static_cast<int>(v.size()), depth);
                    auto elapsed = std::chrono::high_resolution_clock::now() - start;
                    sum_random += std::chrono::duration_cast<std::chrono::microseconds>(elapsed).count();
                }
            ...
            std::cerr << "Introsort: Done n=" << n << " (depth=" << depth << ")\n";
        }
    }
};
